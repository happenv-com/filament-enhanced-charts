...
it('builds a bar series with stack and itemStyle', function (): void {
...
it('applies barWidth on a bar series', function (): void {
    expect(BarSeries::make()->barWidth(20)->data([1])->toArray())
        ->toBe(['type' => 'bar', 'data' => [1], 'barWidth' => 20]);
});

it('lets raw() override a bar series stack', function (): void {
    expect(BarSeries::make()->stack('x')->raw(['stack' => 'y'])->toArray()['stack'])
        ->toBe('y');
});

it('applies barGap and barCategoryGap on a bar series', function (): void {
    expect(BarSeries::make()->barGap('30%')->barCategoryGap(20)->data([1])->toArray())
        ->toEqual(['type' => 'bar', 'data' => [1], 'barGap' => '30%', 'barCategoryGap' => 20]);
});

it('applies showBackground, backgroundStyle and roundCap on a bar series', function (): void {
    expect(
        BarSeries::make()
            ->showBackground()
...
it('applies labelLine on a pie series', function (): void {
...
it('accepts selectedMode as a boolean or a string on a pie series', function (): void {
...
it('applies padAngle and avoidLabelOverlap on a pie series', function (): void {
...
it('lets raw() override a pie series itemStyle', function (): void {
    expect(

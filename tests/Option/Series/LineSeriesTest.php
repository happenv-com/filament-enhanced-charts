...
it('builds a minimal line series with normalized BcMath data', function (): void {
    expect(LineSeries::make()->name('Sale')->data([7, new Number('4.50')])->toArray())
        ->toBe(['type' => 'line', 'name' => 'Sale', 'data' => [7, ['__js__' => '4.5']]]);
});

it('applies smooth, dashed and stack', function (): void {
...
it('merges dashed() into an existing lineStyle instead of clobbering it', function (): void {
    expect(
        LineSeries::make()->lineStyle(LineStyle::make()->color('#f00'))->dashed()->data([1])->toArray()['lineStyle']
    )->toBe(['color' => '#f00', 'type' => 'dashed']);
});

it('keeps the dash regardless of dashed()/lineStyle() call order', function (): void {
...
it('accepts a float smoothness value for smooth', function (): void {
    expect(LineSeries::make()->smooth(0.5)->data([1])->toArray())
        ->toEqual(['type' => 'line', 'data' => [1], 'smooth' => 0.5]);
});

it('emits smooth => false when explicitly disabled', function (): void {
    expect(LineSeries::make()->smooth(false)->data([1])->toArray())
        ->toEqual(['type' => 'line', 'data' => [1], 'smooth' => false]);
});

it('applies step as a boolean or a named position', function (): void {
...
it('applies showAllSymbol on a line series', function (): void {
...
it('builds a DataPoint with itemStyle and label parity with its siblings', function (): void {
    expect(

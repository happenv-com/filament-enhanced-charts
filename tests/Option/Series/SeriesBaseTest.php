...
it('applies sampling, progressive and progressiveThreshold on a series', function (): void {
    expect(
...
it('accepts selectedMode as a boolean or a string on a base series', function (): void {
...
it('applies a per-series tooltip override on a series', function (): void {
    expect(BarSeries::make()->tooltip(['formatter' => '{b}: {c}'])->data([1])->toArray())
        ->toEqual(['type' => 'bar', 'data' => [1], 'tooltip' => ['formatter' => '{b}: {c}']]);
});

it('applies labelLayout as an array or a RawJs callback on a series', function (): void {
...
it('applies labelLine on a series that does not override it', function (): void {
    expect(LineSeries::make()->labelLine(['show' => false])->data([1])->toArray())
        ->toEqual(['type' => 'line', 'data' => [1], 'labelLine' => ['show' => false]]);
});

it('converts a bool labelLine into a show toggle on the base series', function (): void {
    expect(LineSeries::make()->labelLine(false)->data([1])->toArray())
        ->toEqual(['type' => 'line', 'data' => [1], 'labelLine' => ['show' => false]]);
});

it('applies animation controls on a series', function (): void {
    expect(
        BarSeries::make()
            ->animation(false)
...
it('accepts a RawJs callback for animationDuration', function (): void {

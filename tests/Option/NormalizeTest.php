...
it('passes scalars and null through unchanged', function (): void {
...
it('turns a BcMath number into an exact literal marker', function (): void {
    expect(Normalize::value(new Number('19.90')))->toBe(['__js__' => '19.9']);
});

it('turns a RawJs value into a marker', function (): void {
    expect(Normalize::value(RawJs::make('(v)=>v')))->toBe(['__js__' => '(v)=>v']);
});

it('normalizes nested arrays including 2D and BcMath leaves', function (): void {
    expect(Normalize::value([1, new Number('2.50'), [3, new Number('4')]]))
        ->toBe([1, ['__js__' => '2.5'], [3, ['__js__' => '4']]]);
});

it('recurses a top-level Arrayable into its array form', function (): void {
    expect(Normalize::value(collect([1, new Number('2.50')])))
        ->toBe([1, ['__js__' => '2.5']]);
});

it('recurses a top-level Node into its array form', function (): void {
    expect(Normalize::value(DataPoint::make(new Number('5.00'))->name('A')))
        ->toBe(['value' => ['__js__' => '5'], 'name' => 'A']);
});

it('marker-izes a RawJs nested inside an array, as a formatter in series data would appear', function (): void {
    expect(Normalize::value(['series' => [RawJs::make('fn'), 3]]))
        ->toBe(['series' => [['__js__' => 'fn'], 3]]);
});

it('builds a js marker from a RawJs or a bare string via js()', function (): void {
    expect(Normalize::js(RawJs::make('(v)=>v')))->toBe(['__js__' => '(v)=>v']);
    expect(Normalize::js('(v)=>v'))->toBe(['__js__' => '(v)=>v']);
});

it('resolves a Filament color palette to its 500 shade', function (): void {
    expect(Normalize::color([50 => 'light', 500 => 'base', 600 => 'dark']))->toBe('base');
});

it('converts an oklch color to rgb ECharts can manipulate', function (): void {
...
it('passes a non-oklch color through unchanged', function (): void {
